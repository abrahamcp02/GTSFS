...
class CartItem {
  static addToCart(userId, seatId, performanceId, callback) {
    const query = 'INSERT INTO ticketCartLines (user_id, seat_id, performance_id) VALUES (?, ?, ?)';
  ...
  static clearCart(userId, callback) {
    const query = 'DELETE FROM ticketCartLines WHERE user_id = ?';
    db.query(query, [userId], (error, results) => {
      if (error) {
